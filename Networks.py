    ...
    def __init__(self, backbone='resnet50', num_classes=7, pretrained=True, drop_rate=0.5):
        ...
        if backbone == 'resnet18':
            self.feature = nn.Sequential(
                *list(models.resnet18(pretrained).children())[:-1],
                nn.Flatten(),
            ...
            self.feature = nn.Sequential(
                *list(models.resnet50(pretrained).children())[:-1],
                nn.Flatten(),
                nn.Dropout(drop_rate),
                nn.Linear(2048, 512),
            )
            ...
            self.feature = nn.Sequential(
                *list(models.mobilenet_v2(pretrained).children())[:-1],
            ...
            if task == 'source':
                ...
                for i in range(7):
                    ...
                    if len(fea_c) != 0:
                        ...
                        inter_loss += mmd_loss(fea_c, fea_) * w[i]
